...
void DisplayLoadedLibs(PEmuPEB_LDR_DATA Ldr) {
    ...
    for (pEntry = rootEntry->Flink; pEntry != rootEntry; pEntry = pEntry->Flink) {
        ...
        if (LdrIsKnownDLL(pDataEntry->BaseDllName.Buffer)) {
...
3.) Add a debugger that isn't total shit
    ...
    Symbolic
4.) Fix 32/64 thunks
    With type annotation information for every system call, we can avoid needing to special-case thunks and automagically convert parameters.
*/