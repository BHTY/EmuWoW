...
HRESULT LdrOpenFile(LPWSTR lpFileName, HANDLE* pHandle, int isLoadingKnownDLL) {
...
HRESULT LdrMapPE(LPWSTR lpLibFileName, HMODULE* pImageBase, int isLoadingKnownDLL) { //check to make sure the PE format is correct
	HANDLE hFile;
	HANDLE hMapping;
	HRESULT hResult;
	PBYTE pData;
	...
	hResult = LdrOpenFile(lpLibFileName, &hFile, isLoadingKnownDLL);
...
HRESULT LdrLoadPE(LPWSTR lpLibFileName, HMODULE* pImageBase) {
	...
	hResult = LdrMapPE(lpLibFileName, &ImageBase, 0);
	if (hResult != ERROR_SUCCESS) return hResult;
...
HRESULT LdrLoadKnownDLL(LPWSTR lpLibFileName, HMODULE* pImageBase) {
	PBYTE ImageBase;
	HRESULT hResult = LdrMapPE(lpLibFileName, &ImageBase, 1);
	CHAR AnsiLibName[64];
	BOOL flag = FALSE;
...
HRESULT LdrLoadDLL(LPWSTR lpLibFileName, HMODULE* pImageBase) {
	...
	hResult = LdrLoadPE(lpLibFileName, &ImageBase, 0);
	if (hResult != ERROR_SUCCESS) return hResult;
